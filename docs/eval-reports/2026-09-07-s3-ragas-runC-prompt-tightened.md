...
- 日期：2026-09-07
- git：`2ea669c`
- **杠杆② Run C**：唯一变量 = 生成 SYSTEM prompt（收紧：逐论断须有片段原文依据、禁止自补命令/步骤、
...
## 杠杆②目标样例 before→after（Run B → Run C，人工比对缓存答案逐句）
...
- 该 trade-off 与 n=50 的 ±0.1 量级抽样噪声同量级，方向可信但幅度不精确；**收紧以 relevancy
  为代价未能换来 faithfulness 净收益**——杠杆②按当前措辞单独使用不达标
...
- **Cache 语义**：生成缓存 key 不含 prompt 内容——本次杠杆②暴露的坑，未来任何 prompt 变更
